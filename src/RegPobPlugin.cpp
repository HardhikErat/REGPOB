...
void RegPobPlugin::ArmScratchpadEdit(
    ...
    const std::string& initialValue)
{
    ...
    suppressAnnotationSync_ = true;
    flightPlan.GetControllerAssignedData().SetFlightStripAnnotation(
        STRIP_ANNOTATION_EDIT_TARGET,
        target == EditTarget::Registration ? "REG" : "POB");
    suppressAnnotationSync_ = false;

    suppressScratchpadSync_ = true;
    flightPlan.GetControllerAssignedData().SetScratchPadString(initialValue.c_str());
    suppressScratchpadSync_ = false;
}

void RegPobPlugin::ClearScratchpadEdit(EuroScopePlugIn::CFlightPlan flightPlan)
{
    if (!flightPlan.IsValid())
    {
        return;
    ...
    const char* rawInput)
{
    ...
    const std::string registration = Validation::SanitizeRegistration(SafeString(rawInput));
    if (!Validation::IsValidRegistration(registration))
    {
        ClearScratchpadEdit(flightPlan);
        return;
    }
    ...
    const char* rawInput)
{
    ...
    const std::optional<int> pob = Validation::ParsePob(SafeString(rawInput));
    if (!pob.has_value())
    {
        ClearScratchpadEdit(flightPlan);
        return;
    ...
    RECT Area)
{
    ...
    SetASELAircraft(flightPlan);
    ArmScratchpadEdit(flightPlan, EditTarget::Registration, data.registration);
    ...
    RECT Area)
{
    ...
    ArmScratchpadEdit(flightPlan, EditTarget::Pob, initialValue);
...
void RegPobPlugin::OnFlightPlanFlightPlanDataUpdate(EuroScopePlugIn::CFlightPlan FlightPlan)
{
    ...
    if (!data.registrationEdited && data.registration.empty())
    {
        const std::string extracted = RemarkParser::ExtractRegistration(remarks);
        if (!extracted.empty())
        {
            cache_.SetRegistration(callsign, extracted, false);
            data.registration = extracted;
            SyncAnnotationsFromCache(FlightPlan);
        }
    }

    if (!data.pobEdited && data.pob <= 0)
    {
        const int extractedPob = RemarkParser::ExtractPOB(remarks);
        if (extractedPob > 0)
        {
            cache_.SetPob(callsign, extractedPob, false);
            data.pob = extractedPob;
            SyncAnnotationsFromCache(FlightPlan);
        }
    }
    ...
    if (regPobList_.IsValid() && IsDepartureCandidate(FlightPlan))
    {
...
void RegPobPlugin::OnTimer(int Counter)
{
    ...
    if (regPobList_.IsValid() && (Counter % 5 == 0))
    {
        RefreshRegPobList(regPobList_);
    }
}
